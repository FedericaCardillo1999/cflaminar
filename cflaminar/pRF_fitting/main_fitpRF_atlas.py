...
def save_params(model, model_name):
    ...
    if rois.__len__()==1:
        pkl_file = opj(f'{MAIN_PATH}/pRFM/{subject}/ses-02/{denoising}/', f'roi-{rois[0]}_model-{atlas}-{model_name}-{depth}_desc-prf_params_{task}.pkl')       
    else:
        pkl_file = opj(f'{MAIN_PATH}/pRFM/{subject}/ses-02/{denoising}/', f'model-{atlas}-{model_name}-{depth}_desc-prf_params_{task}.pkl')                        
